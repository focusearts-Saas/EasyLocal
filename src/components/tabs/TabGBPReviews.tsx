...
export default function TabGBPReviews({
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      <div className="bg-white/80 dark:bg-[#090d16] border border-[#00ff9d]/20 rounded-2xl p-6 space-y-6 mb-8 shadow-[0_0_30px_rgba(0,255,157,0.02)]">
        <div className="flex items-center gap-2 text-[#00ff9d]">
          <span className="text-xl">🧮</span>
          <h3 className="text-lg font-black text-gray-900 dark:text-white uppercase tracking-wider">Simulador de Notas e Avaliações</h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full bg-gray-50 dark:bg-[#121824] border border-gray-200 dark:border-white/10 rounded-lg px-4 py-2.5 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#00ff9d]"
            />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-200 dark:border-white/5">
          ...
          <div className="bg-gray-50 dark:bg-[#121824] border border-[#00ff9d]/10 rounded-xl p-5 relative overflow-hidden" style={{ boxShadow: '0 0 20px rgba(0, 255, 157, 0.01)' }}>
            ...
            <h4 className="text-5xl font-black text-gray-900 dark:text-white flex items-baseline gap-2">
              {neededGoogle} 
              <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">avaliações de 5★</span>
            </h4>
            <p className="text-xs text-gray-500 mt-2">
              Necessário para atingir nota real de <span className="text-gray-900 dark:text-white font-bold">{(targetRating === 5.0 ? 4.95 : targetRating - 0.05).toFixed(2)}</span>, que o Google arredondará e exibirá como <span className="text-[#00ff9d] font-bold">{targetRating.toFixed(1)}</span>.
            </p>
          </div>

          {/* Card Nota Exata */}
          <div className="bg-gray-50 dark:bg-[#121824] border border-gray-200 dark:border-white/5 rounded-xl p-5">
            ...
            <p className="text-xs text-gray-500 mt-2">
              Necessário para que a média aritmética exata seja rigorosamente igual ou superior a <span className="text-gray-900 dark:text-white font-bold">{targetRating.toFixed(2)}</span>.
            </p>
          </div>
        </div>
      </div>
      {loadingLocal ? (
        ...
        <div className="text-center p-16 border border-dashed border-gray-300 dark:border-gray-800 rounded-2xl text-gray-500 bg-gray-100 dark:bg-[#161b22]/30">🎉 Todas as avaliações foram respondidas!</div>
      ) : (
            ...
            <div key={i} className="glass-card rounded-2xl p-8 flex flex-col border-[#00ff9d]/10" style={{ boxShadow: '0 0 30px rgba(0, 255, 157, 0.05)' }}>
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-4">
                  <img src={review.reviewer?.profilePhotoUrl} alt="" className="w-12 h-12 rounded-full bg-gray-100 dark:bg-[#161b22] border border-gray-200 dark:border-gray-800" />
                  <div>
                    <p className="font-bold text-gray-900 dark:text-white text-lg">{review.reviewer?.displayName}</p>
                ...
                <div className="text-xs font-bold text-gray-500 px-3 py-1 bg-gray-100 dark:bg-[#161b22] rounded-full border border-gray-200 dark:border-gray-800">
                  {review.createTime ? new Date(review.createTime).toLocaleDateString() : ''}
                </div>
              </div>
              <p className="text-gray-600 dark:text-gray-300 text-sm mb-8 leading-relaxed italic flex-1">"{review.comment || '(Avaliação sem comentário)'}"</p>
              {review.reviewReply ? (
                <div className="bg-gray-50 dark:bg-[#161b22] border border-gray-200 dark:border-gray-800 rounded-xl p-5 border-l-4 border-l-[#00ff9d] mt-auto">
                  <p className="text-[10px] text-[#00ff9d] font-bold mb-2 uppercase tracking-widest">Resposta Publicada</p>
                  <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">{review.reviewReply.comment}</p>
                </div>
              ) : (
                <div className="bg-white dark:bg-[#0d1117]/80 border border-[#00ff9d]/20 rounded-xl p-5 mt-auto shadow-[0_0_15px_rgba(0,255,157,0.05)]">
                  ...
                  <textarea
                    ...
                    className="w-full bg-gray-50 dark:bg-[#161b22] border border-gray-200 dark:border-gray-800 text-gray-800 dark:text-gray-200 p-4 rounded-xl text-sm mb-4 focus:outline-none focus:border-[#00ff9d] focus:ring-1 focus:ring-[#00ff9d] min-h-[130px] resize-y font-medium"
                  />
                  <div className="flex flex-col sm:flex-row gap-3 justify-end">
                    <button onClick={() => handleGenerateAI(review)} disabled={generatingAI[review.name]}
                      className="bg-white dark:bg-[#161b22] hover:bg-gray-50 dark:hover:bg-[#161b22]/80 border border-[#00ff9d]/30 text-[#00ff9d] px-5 py-2.5 rounded-lg text-sm font-bold transition-colors flex justify-center items-center gap-2 disabled:opacity-50">
